    ...
    for (var i = 0; i < track_list.length; i++) {
        ...
        for (var j = 0; j < track.length; j++) {
            if (window["track_list" + track[j].name].graph == "true") {
                ...
                to = Math.ceil(parseInt(getEnd()) + partial);
                addJSON(from, to, track[j].name, window["track_list" + track[j].name].id);
            }
    ...
    if ((to - from) > 0) {
        ...
        if (trackName && trackId) {
            Fluxion.doAjax(
                'comparaService',
                'loadTrack',
                {'query': seqregname, 'trackname': trackName, 'trackid': trackId, 'reference': jQuery('#genomes').val(), 'start': from, 'end': to, 'delta': deltaWidth, 'url': ajaxurl},
            ...
            for (var i = 0; i < Tracklist.length; i++) {
                ...
                for (var j = 0; j < track.length; j++) {
                    if (track[j].name == "blasttrack") {
                        trackToggle(track[j].name);
                    }
