 ...
 */
Smits = {};
Smits.Common = {nodeIdIncrement: 0, activeNode: 0, roundFloat: function (a, c) {
    for (var b = 0, f = 1; b < c;)f *= 10, b++;
    return Math.round(a * f) / f
}, apply: function (a, c) {
    if (a && typeof c == "object")for (var b in c)a[b] = c[b];
    return a
    ...
    if (c)for (var b in c)c.hasOwnProperty(b) && a.setAttribute(b, String(c[b]));
    return a
}, createGradientEl: function (a, c, b) {
    if (c.type != "radialGradient")return!1;
    a = Smits.Common.createSvgEl("radialGradient", {id: a, gradientUnits: "userSpaceOnUse", cx: b[0], cy: b[1], r: b[2], fx: b[0], fy: b[1]});
    if (c.stop) {
        c = c.stop;
...
Smits.PhyloCanvas = function () {
    ...
    return function (k, l, p, g, m) {
        ...
        if (typeof k === "object")if (k.xml) {
            var d = k.fileSource ? k.xml : XMLObjectifier.textToXML(k.xml), d = XMLObjectifier.xmlToJSON(d);
...
Smits.PhyloCanvas.Node.prototype = {getCountAllChildren: function () {
    ...
    var a = 0, c;
    for (c in this.children)if (Smits.Common.isInteger(c)) {
...
Smits.PhyloCanvas.NewickParse = function () {
    var a, c, b, f = 0, k = 0, l, p = function (a) {
        for (var e = new Smits.PhyloCanvas.Node; c !== ")" && c !== ",";)if (c === ":") {
        ...
        if (d.children && d.children.length)for (var c = 0; c < d.children.length; c++) {
            var a = d.children[c];
            if (a.len === 0)a.len = 1;
...
Smits.PhyloCanvas.PhyloxmlParse = function () {
    var a = 0, c = 0, b, f, k = function (a, c) {
        var d = new Smits.PhyloCanvas.Node;
        if (c)d.level = c.level + 1;
        if (a.clade && a.clade.length)for (var j = 0; j < a.clade.length; j++)d.children.push(k(a.clade[j], d));
        if (a.branch_length) {
        ...
        if (!d.name) {
            if (d.sequenceName)d.name = d.sequenceName; else if (d.taxonomyScientificName)d.name = d.taxonomyScientificName; else if (d.taxonomyCommonName)d.name = d.taxonomyCommonName; else if (d.sequenceAccession)d.name = d.sequenceAccession;
        ...
        if (a.annotation) {
            ...
            if (a.annotation[0] && a.annotation[0].img)for (j in a.annotation[0].img)if (Smits.Common.isInteger(j))d.img[j] = a.annotation[0].img[j].Text
        }
        if (a.chart && a.chart[0])for (j in a.chart[0])if (j != "Text" && j !=
        ...
        if (b.children && b.children.length)for (var f = 0; f < b.children.length; f++) {
            var d = b.children[f];
...
Smits.PhyloCanvas.NexmlParse = function () {
    var a = 0, c = 0, b, f, k, l, p = function (a, d, c) {
        var i = new Smits.PhyloCanvas.Node;
        if (c)i.level = c.level + 1;
        for (c = 0; c < k.length; c++)if (k[c].source == a.id)for (var e = 0; e < l.length; e++)k[c].target == l[e].id && i.children.push(p(l[e], k[c].length, i));
        ...
        if (b.children && b.children.length)for (var d = 0; d <
            b.children.length; d++) {
            var j = b.children[d];
...
Smits.PhyloCanvas.NexmlJsonParse = function () {
    var a = 0, c = 0, b, f, k = [], l = [], p = function (a, c, i) {
        var e = new Smits.PhyloCanvas.Node;
        if (i)e.level = i.level + 1;
        for (i = 0; i < k.length; i++)if (k[i].source == a.id)for (var b = 0; b < l.length; b++)k[i].target == l[b].id && e.children.push(p(l[b], k[i].length, e));
        ...
        if (d.children && d.children.length)for (var b = 0; b < d.children.length; b++) {
            var i = d.children[b];
    ...
    return function (d) {
        ...
        if (d.tree && j.trees[0] && j.trees[0].tree[d.tree - 1])k = j.trees[0].tree[d.tree - 1].edge, l = j.trees[0].tree[d.tree - 1].node; else {
            for (h =
...
Smits.PhyloCanvas.NexmlParse.prototype = {};
Smits.PhyloCanvas.Render = {};
Smits.PhyloCanvas.Render.Style = {line: {stroke: "rgb(0,0,0)", "stroke-width": 1}, text: {"font-family": "Verdana", "font-size": 12, "text-anchor": "start"}, path: {stroke: "rgb(0,0,0)", "stroke-width": 1}, connectedDash: {stroke: "rgb(200,200,200)", "stroke-dasharray": ". "}, textSecantBg: {fill: "#EEE", stroke: "#DDD"}, highlightedEdgeCircle: {fill: "red"}, barChart: {fill: "#003300", stroke: "#DDD"}, getStyle: function (a, c) {
...
Smits.PhyloCanvas.Render.Parameters = {jsOverride: 0, Rectangular: {bufferX: 200, paddingX: 10, paddingY: 20, bufferInnerLabels: 10, bufferOuterLabels: 5, minHeightBetweenLeaves: 10, alignPadding: 0, alignRight: !1, showScaleBar: !1}, Circular: {bufferRadius: 0.33, bufferAngle: 20, initStartAngle: 160, innerCircleRadius: 0, minHeightBetweenLeaves: 5, bufferInnerLabels: 2, bufferOuterLabels: 5}, binaryCharts: [], integratedBinaryCharts: [], barCharts: [], binaryChartBufferInner: 5, binaryChartBufferSiblings: 0.01, binaryChartThickness: 15, binaryChartDisjointed: !1,
    barChartBufferInner: 3, barChartHeight: 50, barChartWidth: 0.5, mouseRollOver: function (a) {
        if (a.node.edgeCircleHighlight)a.node.edgeCircleHighlight.show(); else {
            var c = a.svg.draw(new Smits.PhyloCanvas.Render.Circle(a.x, a.y, 5, {attr: Smits.PhyloCanvas.Render.Style.highlightedEdgeCircle}));
            a.node.edgeCircleHighlight = c[0]
        }
        a.textEl.attr({fill: "red"})
    }, mouseRollOut: function (a) {
        a.node.edgeCircleHighlight.hide();
        a.textEl.attr({fill: "#000"})
    }, set: function (a, c, b) {
...
Smits.PhyloCanvas.Render.SVG.prototype = {render: function () {
    ...
    for (var c = 0; c < a.length; c++)if (a[c].type == "line")this.svg.path(["M", a[c].x1, a[c].y1, "L", a[c].x2, a[c].y2]).attr(Smits.PhyloCanvas.Render.Style.line); else if (a[c].type == "path")this.svg.path(a[c].path).attr(a[c].attr); else if (a[c].type == "circle")this.svg.circle(a[c].x, a[c].y, a[c].radius).attr({stroke: "red"}); else {
...
Smits.PhyloCanvas.Render.Phylogram = function () {
    var a, c = Smits.PhyloCanvas.Render.Parameters.Rectangular, b, f, k, l, p, g, m = !0, d = 0, j = 0, i, e, h, n, o, q = [], u = function (b, e) {
        ...
        if (b.children.length > 0) {
            ...
            if (b.name) {
                ...
                a.draw(new Smits.PhyloCanvas.Render.Text((f || e) + 5, g, b.name, {attr: h}))
            }
            if (b.children && b.children.length)for (h = 0; h < b.children.length; h++)i.push(u(b.children[h], e));
            f = [];
            for (h = 0; h < i.length; h++)i[h][0] && f.push(i[h][0]), i[h][1] && f.push(i[h][1]);
            i = Math.min.apply(null, f);
            f = Math.max.apply(null, f);
            a.draw(new Smits.PhyloCanvas.Render.Path(["M", e + 1.0E-4, i, "L", e, i, "L", e, f, "L", e + 1.0E-4, f], {attr: Smits.PhyloCanvas.Render.Style.line}))
        } else {
            h =
                e;
            ...
            c.alignRight && a.draw(new Smits.PhyloCanvas.Render.Path(["M", f, o, "L", c.alignPadding + p, n], {attr: Smits.PhyloCanvas.Render.Style.connectedDash}));
            if (b.name) {
                h = {};
                ...
                i = a.draw(new Smits.PhyloCanvas.Render.Text(c.alignRight ? p + c.bufferInnerLabels +
                    c.alignPadding : f + c.bufferInnerLabels, n, b.name, {attr: h}));
                j = Math.max(i[1], j);
                Smits.PhyloCanvas.Render.Parameters.mouseRollOver && Smits.Common.addRaphEventHandler(i[0], "mouseover", Smits.PhyloCanvas.Render.Parameters.mouseRollOver, {svg: a, node: b, x: f, y: n, textEl: i[0]});
                Smits.PhyloCanvas.Render.Parameters.mouseRollOut && Smits.Common.addRaphEventHandler(i[0], "mouseout", Smits.PhyloCanvas.Render.Parameters.mouseRollOut, {svg: a, node: b, x: f, y: n, textEl: i[0]});
                Smits.PhyloCanvas.Render.Parameters.onClickAction && Smits.Common.addRaphEventHandler(i[0],
                    "click", Smits.PhyloCanvas.Render.Parameters.onClickAction, {svg: a, node: b, x: f, y: n, textEl: i[0]})
            }
            m && (m = !1)
        }
        return[o, n]
        ...
        for (var e = (b && b.bufferInner ? b.bufferInner : 0) | Smits.PhyloCanvas.Render.Parameters.binaryChartBufferInner, i = (b && b.bufferSiblings ? b.bufferSiblings * l : 0) | (Smits.PhyloCanvas.Render.Parameters.binaryChartBufferSiblings < 1 ? l * Smits.PhyloCanvas.Render.Parameters.binaryChartBufferSiblings : Smits.PhyloCanvas.Render.Parameters.binaryChartBufferSiblings), b = (b && b.thickness ? b.thickness :
            ...
            var j = q[h];
            a.draw(new Smits.PhyloCanvas.Render.Path(["M", d + e, j.y - l / 2 + i / 2, "L", d + e + b, j.y - l / 2 + i / 2, "L", d + e + b, j.y + l / 2 - i / 2, "L", d + e, j.y + l / 2 - i / 2, "Z"], {attr: Smits.PhyloCanvas.Render.Style.getStyle(j.chart[c], "textSecantBg")}))
        }
        return d + e + b
    }, B = function (d, c, b) {
        ...
        for (f = 0; f < q.length; f++)e = q[f], a.draw(new Smits.PhyloCanvas.Render.Path(["M", d + i, e.y - b / 2, "L", d + i + j * e.chart[c], e.y - b / 2, "L", d + i + j * e.chart[c], e.y + b / 2, "L", d + i, e.y + b / 2, "Z"], {attr: Smits.PhyloCanvas.Render.Style.getStyle(e.chart[c],
            "barChart")}));
        return d + i + h
    };
    return function (q, m) {
        ...
        a = q;
        var r = m.getRoot(), v = m.getNewickLen();
        b = a.canvasSize[0];
        f = a.canvasSize[1];
        h = c.bufferX;
        n = c.paddingX;
        ...
        if (Smits.PhyloCanvas.Render.Parameters.binaryCharts.length) {
            var r = Smits.PhyloCanvas.Render.Parameters.binaryCharts, s;
            for (s in r)i = z(i, r[s].chart, r[s])
        }
        if (Smits.PhyloCanvas.Render.Parameters.barCharts.length)for (s in r = Smits.PhyloCanvas.Render.Parameters.barCharts,
            r)B(i, r[s].chart, r[s])
    }
}();
...
Smits.PhyloCanvas.Render.CircularPhylogram = function () {
    ...
    function b(d, b, c, e) {
        var i = a(d, b), h = a(d, c), j = [], f = 0, b = Math.abs(k(c - b)) > 180 ? 1 : -1;
    ...
    function l(a, d, c, e) {
        ...
        return g("M", b(a, c, e, {noMove: 1, invertSecant: 0}), "L", b(d, e, c, {noMove: 1, invertSecant: 1}), "Z")
    }
    ...
    function p(d, h) {
        ...
        if (d.children.length > 0) {
            ...
            if (d.children && d.children.length)for (var l =
            ...
            d.level != 0 && i.draw(new Smits.PhyloCanvas.Render.Path(g("M", a(h + 0.01, l), "L", b(h, l, j, {noMove: !0}), "L", a(h + 0.01, j))));
            ...
            n))), i.draw(new Smits.PhyloCanvas.Render.Path(f(k, n, v), {attr: Smits.PhyloCanvas.Render.Style.connectedDash})), d.name) {
            ...
            j = i.draw(new Smits.PhyloCanvas.Render.Text(o[0], o[1], d.name, {attr: m, rotate: [j, o[0], o[1]]}));
            d.bgStyle && t.push([d.bgStyle, k]);
            o = a(n, k);
            Smits.PhyloCanvas.Render.Parameters.mouseRollOver &&
            Smits.Common.addRaphEventHandler(j[0], "mouseover", Smits.PhyloCanvas.Render.Parameters.mouseRollOver, {svg: i, node: d, x: o[0], y: o[1], textEl: j[0]});
            Smits.PhyloCanvas.Render.Parameters.mouseRollOut && Smits.Common.addRaphEventHandler(j[0], "mouseout", Smits.PhyloCanvas.Render.Parameters.mouseRollOut, {svg: i, node: d, x: o[0], y: o[1], textEl: j[0]});
            Smits.PhyloCanvas.Render.Parameters.onClickAction && Smits.Common.addRaphEventHandler(j[0], "click", Smits.PhyloCanvas.Render.Parameters.onClickAction, {svg: i, node: d, x: o[0], y: o[1],
                textEl: j[0]});
            x = Math.max(j[1], x)
        }
        B && (B = !1);
    ...
    function m() {
        var a = [];
        if (t.length > 0) {
            if (Smits.PhyloCanvas.Render.Style.jsphylosvgGradientList)for (var d = 0; d < Smits.PhyloCanvas.Render.Style.jsphylosvgGradientList.length; d++) {
                var b = Smits.PhyloCanvas.Render.Style.jsphylosvgGradientList[d], b = Smits.Common.createGradientEl(b, Smits.PhyloCanvas.Render.Style[b], [E, r, v + x + e.bufferOuterLabels]);
                i.svg.defs.appendChild(b)
            }
            for (d =
                     0; d < t.length; d++)d != t.length - 1 && t[d][0] == t[d + 1][0] ? t[d + 1][2] = t[d][2] ? t[d][2] : t[d][1] : (a = l(v, v + x + e.bufferOuterLabels, t[d][2] ? t[d][2] - u / 2 : t[d][1] - u / 2, t[d][1] + u / 2), b = Smits.PhyloCanvas.Render.Style.getStyle(t[d][0], "textSecantBg"), a = i.draw(new Smits.PhyloCanvas.Render.Path(a, {attr: b.type ? {} : b})), b.type && b.type == "radialGradient" && a[0].node.setAttribute("fill", "url(#" + b.name + ")"), b.type && b.type == "radialGradient" && a[0].node.setAttribute("stroke", "none"), a[0].toBack())
        }
        a = l(v, v + x + e.bufferOuterLabels, (C || 1) -
            u / 2, 360 - u / 2);
        a = i.draw(new Smits.PhyloCanvas.Render.Path(a, {attr: Smits.PhyloCanvas.Render.Style.textSecantBg}));
        a[0].toBack();
    ...
    function d(d, b, e) {
        for (var j = e && e.bufferInner ? parseFloat(e.bufferInner) : Smits.PhyloCanvas.Render.Parameters.binaryChartBufferInner, h = (e && e.bufferSiblings ? e.bufferSiblings * u : 0) | (Smits.PhyloCanvas.Render.Parameters.binaryChartBufferSiblings < 1 ? u * Smits.PhyloCanvas.Render.Parameters.binaryChartBufferSiblings : Smits.PhyloCanvas.Render.Parameters.binaryChartBufferSiblings),
            ...
            if ((!s[m + 1] || q.chart[b] !== s[m + 1].chart[b] || o) && q.chart[b] != "none") {
                ...
                if (p.label) {
                    var n = Smits.PhyloCanvas.Render.Style.getStyle(p.labelStyle, "text"), q = a((g[0] + g[1]) / 2, (g[2] + g[3]) / 2), r = c((g[2] + g[3]) / 2), r = k(r[0] + (n.rotate ? parseFloat(n.rotate) : 0)), t = k(90 - (g[2] + g[3]) / 2 - D);
                    t > 90 && t < 270 && (r += 180);
                    n["text-anchor"] || (n["text-anchor"] = "middle");
                    i.draw(new Smits.PhyloCanvas.Render.Text(q[0], q[1], p.label, {attr: n, rotate: r}))[0].toBack()
                }
                p.borderStyle && (n = Smits.PhyloCanvas.Render.Style.getStyle(p.borderStyle, "textSecantBg"), i.draw(new Smits.PhyloCanvas.Render.Path(l([v,
                    n.fullsize ? g[1] : g[0], g[2], g[3]]), {attr: n}))[0].toBack());
                i.draw(new Smits.PhyloCanvas.Render.Path(l(g), {attr: p}))[0].toBack();
                g = 0
            } else {
                if (!g)g = q.y;
                q.chart[b] == "none" && (g = 0)
    ...
    function j(d, a, b) {
        ...
        for (f = 0; f < s.length; f++)c = s[f], c.chart[a] > 0 && i.draw(new Smits.PhyloCanvas.Render.Path(l(d + e, d + e + h * c.chart[a], c.y - b / 2, c.y + b / 2), {attr: Smits.PhyloCanvas.Render.Style.getStyle(c.chart[a], "barChart")}));
        return d +
            e + j
    }

    var i, e = Smits.PhyloCanvas.Render.Parameters.Circular, h, n, o, q, u, z, B = !0, A = 0, E, r, v, s = [], t = [], C, w, x = 0, D, F = Math.PI / 180;
    return function (a, b, c) {
        ...
        if (b.getValidate())a.draw({type: "text", x: 0, y: a.canvasSize[1] / 3, text: b.getValidate()}); else {
            i = a;
            var a = b.getRoot(), f = b.getNewickLen();
            h = i.canvasSize[0];
            n = i.canvasSize[1];
            E = h / 2;
            r = n / 2;
            ...
            if (Smits.PhyloCanvas.Render.Parameters.integratedBinaryCharts.length) {
                var c = Smits.PhyloCanvas.Render.Parameters.integratedBinaryCharts, g;
            ...
            w = m();
            if (Smits.PhyloCanvas.Render.Parameters.binaryCharts.length)for (g in c = Smits.PhyloCanvas.Render.Parameters.binaryCharts, c)w = d(w, c[g].chart, c[g]);
            if (Smits.PhyloCanvas.Render.Parameters.barCharts.length)for (g in c = Smits.PhyloCanvas.Render.Parameters.barCharts, c)w = j(w, c[g].chart, c[g])
        }
    }
}();
...
var XMLObjectifier = function () {
    ...
    return{xmlToJSON: function (c) {
        try {
            if (!c)return null;
            var b = {typeOf: "JSXBObject"}, f = c.nodeType == 9 ? c.documentElement : c;
                ...
                return{activate: function () {
                    ...
                    a.getNodeByAttribute = function (b, c) {
                        if (a && a.length > 0) {
                            var e, h = a.length - 1;
                            try {
                                do if (e = a[h], e[b] === c)return e; while (h--)
                            } catch (f) {
                            }
                            return null
                        }
                    ...
                    a.contains = function (b, c) {
                        if (a && a.length > 0) {
                            var e = a.length - 1;
                            try {
                                do if (a[e][b] === c)return!0; while (e--)
                            } catch (h) {
                            }
                            return!1
                        }
            ...
            var p = function (b) {
                b.getNodeByAttribute = function (a, b) {
                    if (this.length > 0) {
                        ...
                        try {
                            do if (d = this[c], d[a] == b)return d; while (c--)
                        } catch (f) {
                        }
                        return!1
                    }
                ...
                b.contains = function (a, b) {
                    if (this.length > 0) {
                        var d = this.length - 1;
                        try {
                            do if (this[d][a] == b)return!0; while (d--)
                        } catch (c) {
                        }
                        return!1
                    }
                ...
                a.Text = "";
                if (b.hasChildNodes()) {
                    var o = b.childNodes.length - 1, q = 0;
